...
local function find_claude_tab()
	...
	for _, tab in ipairs(tabs) do
		...
		for _, win in ipairs(wins) do
			...
			if buf_name:match("claude%-code") or (state and state.bufnr and buf == state.bufnr) then
				return tab, win
			end
		end
	end
...
local function create_claude_buffer()
	...
	elseif win_config.type == "tabnew" then
		...
		if existing_tab then
			...
			-- Create buffer first with a name, then use it for the new tab
			state.bufnr = vim.api.nvim_create_buf(false, true)
	...
	-- Keep buffer alive when hidden (don't wipe)
	...
	-- Disable plugins that might interfere with terminal buffer
	vim.schedule(function()
		if vim.api.nvim_buf_is_valid(state.bufnr) then
			-- Disable treesitter
			vim.b[state.bufnr].ts_highlight = false
			pcall(vim.treesitter.stop, state.bufnr)
			
			-- Disable render-markdown
			vim.b[state.bufnr].render_markdown = false
			
			-- Set buffer as terminal type to prevent other plugins from interfering
	...
	-- Allow exiting terminal mode with Esc or Ctrl-\ Ctrl-n
	...
	-- Ensure plugins stay disabled for this buffer
	vim.api.nvim_create_autocmd({"BufEnter", "WinEnter"}, {
		buffer = state.bufnr,
		callback = function()
			vim.b[state.bufnr].ts_highlight = false
			vim.b[state.bufnr].render_markdown = false
			if vim.bo[state.bufnr].filetype ~= 'terminal' then
				vim.bo[state.bufnr].filetype = 'terminal'
			end
		end,
	})
...
function M.open()
	-- Only reset session info if no existing buffer
		...
		if state.winnr and vim.api.nvim_win_is_valid(state.winnr) then
			...
			elseif win_config.type == "tabnew" then
				...
				if existing_tab then
					...
					-- Create new tab with existing buffer
					vim.cmd("tabnew")
					state.winnr = vim.api.nvim_get_current_win()
					vim.api.nvim_win_set_buf(state.winnr, state.bufnr)
				end
	...
	if state.winnr and vim.api.nvim_win_is_valid(state.winnr) then
		...
		if win_config.type == "tabnew" then
			...
			if #tab_wins == 1 and tab_wins[1] == state.winnr then
				-- If this is the only window in the tab
				if num_tabs > 1 then
					-- Safe to close the tab if there are other tabs
					vim.cmd("tabclose")
				else
					-- This is the last tab, can't close it, so just hide the buffer
					vim.cmd("enew")
				end
			else
			...
			-- For splits, check if it's the last window
			local wins = vim.api.nvim_list_wins()
			if #wins > 1 then
				vim.api.nvim_win_close(state.winnr, true)
			else
				-- Last window, can't close it, so just switch to a new buffer
				vim.cmd("enew")
			end
		end
		state.winnr = nil
